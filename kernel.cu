...
__global__ void k_calcFftBatched1D(ComplexVar* data, const bool inverse = false) {
    ...
    constexpr unsigned int gridSteps = (N + BLOCKS - 1) / BLOCKS;
    constexpr unsigned int blockSteps = (N + THREADS - 1) / THREADS;
...
constexpr int NUM_SUB_MATRICES_PER_DIMENSION = N / SUB_MATRIX_SIZE;
...
template<int N>
__global__ void k_generateFilterLattice(ComplexVar* data) {
    const int thread = threadIdx.x;
    const int block = blockIdx.x;
    const int numBlocks = gridDim.x;
    const int numThreads = blockDim.x;
...
template<int N>
__global__ void k_forceMultiSampling(ComplexVar* lattice_d, float* x, float* y, float* vx, float* vy, const int numParticles) {
    const int thread = threadIdx.x;
    const int block = blockIdx.x;
    const int numBlocks = gridDim.x;
    const int numThreads = blockDim.x;
    ...
    for (int ii = 0; ii < steps; ii++) {
        ...
        if (index < numParticles) {
            ...
            if (centerX >= 0 && centerX < N && centerY >= 0 && centerY < N) {
                ...
                if (centerY - 1 >= 0) {
                    top = lattice_d[centerIndex - N].x;
                }
                if (centerY + 1 < N) {
                    bot = lattice_d[centerIndex + N].x;
                }
...
template<int N>
__global__ void k_shiftLattice(ComplexVar* lattice_d, ComplexVar* latticeShifted_d) {
    const int thread = threadIdx.x;
    const int block = blockIdx.x;
    const int numBlocks = gridDim.x;
    const int numThreads = blockDim.x;
    ...
    for (int ii = 0; ii < steps; ii++) {
        ...
        if (index < N * N) {
            ...
            latticeShifted_d[index] = lattice_d[shiftedX + shiftedY * N];
        }
    }
}
template<int N>
__global__ void k_scatterMassOnLattice(ComplexVar* lattice_d, const float* x, const float* y, const int numParticles) {
    const int thread = threadIdx.x;
    const int block = blockIdx.x;
    const int numBlocks = gridDim.x;
    const int numThreads = blockDim.x;
    ...
    for (int ii = 0; ii < steps; ii++) {
        ...
        if (index < numParticles) {
            ...
            if (xi >= 0 && xi < N && yi >= 0 && yi < N) {
                atomicAdd(&lattice_d[xi + yi * N].x, 1.0f);
            }
        }
    }
}
...
struct Universe {
    ...
    Universe(int particles = 0, int tileSizeParameter = 32) {
        ...
        lattice.resize(N * N);
        mat = cv::Mat(cv::Size2i(N, N), CV_32FC1);
        const int centerX = N / 2;
        const int centerY = N / 2;
        const float speed = 2.50f;
        for (int i = 0; i < particles; i++) {
            const float r = rand() % (N / 3);
            const float a = MATH_PI * 2.0 * (rand() % 1000) / 1000.0f;
            x[i] = r * cos(a) + N / 2;
            y[i] = r * sin(a) + N / 2;
        ...
        gpuErrchk(cudaMalloc(&lattice_d, sizeof(ComplexVar) * N * N));
        gpuErrchk(cudaMalloc(&latticeShifted_d, sizeof(ComplexVar) * N * N));
        gpuErrchk(cudaMalloc(&filter_d, sizeof(ComplexVar) * N * N));
        ...
        cudaFuncSetAttribute(k_calcFftBatched1D<N>, cudaFuncAttributePreferredSharedMemoryCarveout, cudaSharedmemCarveoutMaxShared);
        cudaFuncSetAttribute(k_calcFftBatched1D<N>, cudaFuncAttributeMaxDynamicSharedMemorySize, N * sizeof(ComplexVar));
        k_fillCoefficientArray<N> << <1, 1024 >> > ();
        k_fillPairIndexList<N> << <BLOCKS, THREADS >> > ();
        k_generateFilterLattice<N> << <BLOCKS, THREADS >> > (filter_d);
    ...
    void scatterMassOnLattice() {
        k_clearLattice<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_scatterMassOnLattice<N> << <BLOCKS, THREADS >> > (lattice_d, x_d, y_d, numParticles);

    }
    void calcLatticeFft2D() {
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (lattice_d, false);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_calcTransposeDiagonals<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (lattice_d, false);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_calcTransposeDiagonals<N> << <BLOCKS, THREADS >> > (lattice_d);
    ...
    void calcFilterFft2D() {
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (filter_d, false);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (filter_d);
        k_calcTransposeDiagonals<N> << <BLOCKS, THREADS >> > (filter_d);
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (filter_d, false);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (filter_d);
        k_calcTransposeDiagonals<N> << <BLOCKS, THREADS >> > (filter_d);
    ...
    void multiplyLatticeFilterElementwise() {
        k_multElementwiseLatticeFilter<N> << <BLOCKS, THREADS >> > (lattice_d, filter_d);
    }
    void calcLatticeIfft2D() {
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (lattice_d, true);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_calcTransposeDiagonals<N> << <BLOCKS, THREADS >> > (lattice_d);
        k_calcFftBatched1D<N> << <BLOCKS, THREADS, N * sizeof(ComplexVar) >> > (lattice_d, true);
        k_calcTranspose<N> << <BLOCKS, THREADS >> > (lattice_d);
    ...
    void multiSampleForces() {
        k_shiftLattice<N> << <BLOCKS, THREADS >> > (lattice_d, latticeShifted_d);
        k_forceMultiSampling<N> << <BLOCKS, THREADS >> > (latticeShifted_d, x_d, y_d, vx_d, vy_d, numParticles);
    }
    void stopBenchmark() {
    ...
    void render() {
        ...
        gpuErrchk(cudaMemcpy(lattice.data(), lattice_d, sizeof(ComplexVar) * N * N, cudaMemcpyDeviceToHost));
        for (int i = 0; i < N * N; i++) {
            mat.at<float>(i) = lattice[i].x;
        }


        cv::imshow("Fast Nbody", mat);
        cv::waitKey(1);
    }
    ~Universe() {
...
int main() {
    const int numNbodySimulationsPerRender = 10;
    srand(time(0));
